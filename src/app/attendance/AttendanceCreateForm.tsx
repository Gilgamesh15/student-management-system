...
const AttendanceCreateForm = ({ students }: AttendanceCreateFormProps) => {
  ...
  const form = useForm<z.infer<typeof attendanceCreateSchema>>({
    ...
    defaultValues: attendanceCreateDefaultValues,
  });
    ...
    if (state?.errors) {
      Object.entries(state.errors).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          form.setError(key as keyof z.infer<typeof attendanceCreateSchema>, {
            message: value.join(", "),
          });
        }
      });
